    ...
    if (obs_range(j)~=0)&&(~isnan(obs_range(j)))&& j<=size(eph_info.a_f0,1)...
        ...
        if ~isempty(tidx) && Strength(j)>=p.sig_strg
            ...
            if ~isnan(sat.pos_ecef(1))
                ...
                satlog.dp_range(j) = dp_range(j)-f*sat.dt_sv_drift;
            end
        end
    end
end
