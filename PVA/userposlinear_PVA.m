function [pos,msr_res,GDOP,nsv,dnsv,nprior,dnprior,postxhat,postxhatcovar,...
    delta_x,Jydiag,by,H_pos,outliervec,outlierbin,res_std] = ...
...
[pos.LS{1},msr_res.LS{1},GDOP.LS,nsv.LS,dnsv.LS,nprior.LS,dnprior.LS,postxhat.LS,postxhatcovar.LS,...
    delta_x.LS,Jydiag.LS,by.LS{1},H_pos.LS{1},res_std.LS{1}]= LSlinear_PVA(p,cpt,grdpos,x_prior.LS,P_prior.LS);

if p.eb_LTS == 1
for idx = 1:p.LTSn
    [pos.LTS{idx},msr_res.LTS{idx},GDOP.LTS(idx),nsv.LTS(idx),dnsv.LTS(idx),nprior.LTS(idx),...
    dnprior.LTS(idx),postxhat.LTS{idx},postxhatcovar.LTS{idx},delta_x.LTS{idx},Jydiag.LTS{idx},by.LTS{idx},H_pos.LTS{idx},res_std.LTS{idx}]= ...
...
for idx = 1:p.RAPSn
    ...
    dnprior.RAPS(idx),postxhat.RAPS{idx},postxhatcovar.RAPS{idx},delta_x.RAPS{idx},Jydiag.RAPS{idx},by.RAPS{idx},H_pos.RAPS{idx},res_std.RAPS{idx}]= ...
...
for idx = 1:p.TDn
    [pos.TD{idx},msr_res.TD{idx},GDOP.TD(idx),nsv.TD(idx),dnsv.TD(idx),nprior.TD(idx),...
        dnprior.TD(idx),postxhat.TD{idx},postxhatcovar.TD{idx},delta_x.TD{idx},Jydiag.TD{idx},by.TD{idx},H_pos.TD{idx},res_std.TD{idx}]= ...
...
for idx = 1:p.MShbn
    ...
    dnprior.MShb(idx),postxhat.MShb{idx},postxhatcovar.MShb{idx},delta_x.MShb{idx},Jydiag.MShb{idx},by.MShb{idx},H_pos.MShb{idx},res_std.MShb{idx}]=...
...
for idx = 1:p.MStkn
    ...
    dnprior.MStk(idx),postxhat.MStk{idx},postxhatcovar.MStk{idx},delta_x.MStk{idx},Jydiag.MStk{idx},by.MStk{idx},H_pos.MStk{idx},res_std.MStk{idx}]= ...
