...
%--------------------------------------------------------------------------
epsil  = geteps(epsab);
eps_px = epsil; eps_py = epsil; eps_pz = epsil;
Pu_pos = [eps_px^2 eps_py^2 eps_pz^2];
eps_vx = geteps([0.5 95]); eps_vy = eps_vx; eps_vz = eps_vx;
Pu_vel = [eps_vx^2 eps_vy^2 eps_vz^2];
eps_ax = geteps([0.5 95]); eps_ay = eps_ax; eps_az = eps_ax;
Pu_acl = [eps_ax^2 eps_ay^2 eps_az^2];
Pu_pva = [Pu_pos, Pu_vel, Pu_acl];
% eps_clk = geteps([3.5e+05 70]);
% eps_ISB = repmat(geteps([3.5e+05 70]),1,4); % gps, glo, gal, bds
eps_clk = geteps([0.5 95]);
eps_ISB = repmat(geteps([0.5 95]),1,4);
Pu_clk = [eps_clk^2 eps_ISB(ind_s(2:end)).^2];
Pu_drift = geteps([0.5 90])^2;
...
while ~isidentical
    iter = iter + 1;
    [delta_x1,by,~,xflag] = RAPS([res_R;res_D],H_os,P_prior,yCov,E_P,E_R,...
        mu_x,J_lwr_bound,delta_x_old,iter,p);
...
dx2 = ((H_os2'*H_os2)^(-1)) * (H_os2'*[res_R;res_D]);
%------------------------%
pos2 = x_prior(1:3) + dx2(1:3);
